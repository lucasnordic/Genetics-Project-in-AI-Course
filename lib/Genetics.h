...
class Genetics {
	...
	static const int MPC_MIN_GENES = 4;			// minimum allowed genes for MPC
	static const int GFX_GENS_PER_FRAME = 1;	// default 1  ;
	static const int TIME_TO_SLEEP = 0;			// default 0  ;milliseconds ; slow down generation
	static const int PARENTS = .6 * M_CH;		// default .7
	static const int INIT_MUTATIONS = M_CI*20;  // amount of mutations on initial pop
public:
	Genetics();
